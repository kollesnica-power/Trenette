...
import com.example.kollesnica_power.trenette.App;
...
public class RandomTransitionGenerator implements TransitionGenerator {
    ...
    public RandomTransitionGenerator() {
        this(App.getInstance().getResources().getInteger(R.integer.switch_delay), new AccelerateDecelerateInterpolator());
    }
    ...
    public Transition generateNextTransition(RectF drawableBounds, RectF viewport) {
        ...
        if (dstRect == null || drawableBoundsChanged || viewportRatioChanged) {
            srcRect = generateDefaultSrcRect(drawableBounds, viewport);
        } else {
        ...
        dstRect = generateDefaultDestRect(drawableBounds, viewport);
    ...
    private RectF generateDefaultDestRect(RectF drawableBounds, RectF viewportRect) {
        float drawableRatio = MathUtils.getRectRatio(drawableBounds);
        float viewportRectRatio = MathUtils.getRectRatio(viewportRect);
        RectF maxCrop;
        ...
        float width = maxCrop.width() * 0.5f;
        float height = maxCrop.height() * 0.5f;
        int widthDiff = (int) (drawableBounds.width() - width);
        int heightDiff = (int) (drawableBounds.height() - height);
        int left = widthDiff > 0 ? widthDiff : 0;
        int top = heightDiff > 0 ? heightDiff : 0;
        return new RectF(left , 0, left + width , height);
    }

    private RectF generateDefaultSrcRect(RectF drawableBounds, RectF viewportRect) {
        float drawableRatio = MathUtils.getRectRatio(drawableBounds);
        float viewportRectRatio = MathUtils.getRectRatio(viewportRect);
        RectF maxCrop;
